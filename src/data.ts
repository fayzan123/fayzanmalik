...
export const projects: Project[] = [
  ...
  },
  {
    ...
    oneLiner: 'n8n for Claude Code — a visual desktop app with 1,500+ npm downloads for composing multi-agent workflows without touching a single YAML file.',
    description: 'Built a visual workflow composer for Claude Code multi-agent pipelines with 1,500+ npm downloads. Drag agents onto a canvas, wire handoffs with trigger conditions, attach skills, and export a fully working orchestrator skill directly into your Claude installation. The exporter writes agent .md files and a skill with disable-model-invocation: true via BFS traversal of the node graph into natural-language steps. Includes conflict detection via ownership comments, reference vs bespoke node distinction, full file preview before writing anything, and 89 tests across 16 files. Run with npx claude-cwc — no code signing, no friction.',
