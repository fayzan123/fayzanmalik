...
export const projects: Project[] = [
  ...
  },
  {
    ...
    oneLiner: 'Merged a LinkedIn Content Creator agent into the #1 trending GitHub repo (100,000+ stars).',
    description: 'Contributed a LinkedIn Content Creator agent to agency-agents (PR #129), the #1 trending repository on GitHub at the time with 100,000+ stars. Studied existing agents to match the format exactly, built the agent with real templates, tested it on a live use case, and navigated the full fork → branch → PR workflow. The agent handles LinkedIn thought leadership content strategy, post drafting, and audience engagement workflows.',
  ...
  },
  {
    slug: 'claude-check',
    name: 'claude-check',
    oneLiner: 'A CLI tool with 780+ users that scores your prompt before you send it to Claude.',
    description: "An npm CLI with 780+ users that analyses any Claude prompt for complexity, estimates how many messages it'll take, assesses interrupt risk, and returns a safe/caution/do-not-start verdict. It checks your real-time claude.ai usage so you don't burn your limit on a task that'll get cut off halfway.",
  ...
  },
  {
    ...
    oneLiner: 'n8n for Claude Code — a visual desktop app with 3,200+ users and 20+ GitHub stars for composing multi-agent workflows without touching a single YAML file.',
    description: 'Built a visual workflow composer for Claude Code multi-agent pipelines with 3,200+ users and 20+ GitHub stars. Drag agents onto a canvas, wire handoffs with trigger conditions, attach skills, and export a fully working orchestrator skill directly into your Claude installation. The exporter writes agent .md files and a skill with disable-model-invocation: true via BFS traversal of the node graph into natural-language steps. Includes conflict detection via ownership comments, reference vs bespoke node distinction, full file preview before writing anything, and 89 tests across 16 files. Run with npx claude-cwc — no code signing, no friction.',
